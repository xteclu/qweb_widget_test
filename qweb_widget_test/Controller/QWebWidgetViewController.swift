...
//    }
  ...
  open override func viewDidLoad() {
    ...
    pipView = UIPipView(frame: view.bounds)
...
//    }
    ...
    func setUpPip() {
        NotificationCenter.default.addObserver(self, selector: #selector(goToBackground), name: UIApplication.didEnterBackgroundNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(goToForeground), name: UIApplication.didBecomeActiveNotification, object: nil)
    }
    
   
    @objc func goToForeground() {
        print("Entered to foreground")
        if #available(iOS 13.0, *) {
            if let scene = view.window?.windowScene, scene.activationState == .foregroundActive {
                ...
                if isPipActive {
                    ...
                    print("Pip still working, are you stupid WTF!!!")
                }
    ...
    @objc func goToBackground() {
        print("Entered to background")
            if !self.isPipActive {
                if self.pipView.isPipControllerPictureInPicturePossible() {
                    if let rootView = self.webView.superview, self.pipView != rootView {
                        self.pipView.frame = rootView.bounds
                        self.pipView.backgroundColor = .clear
                        self.pipView.isOpaque = false
                        self.view.addSubview(self.pipView)
                        self.pipView.addSubview(self.webView)
                    }
                    self.pipView.startPictureInPicture(withRefreshInterval: (60.0))
                    self.isPipActive = true
                    print("need to be started pip")
                    print("Possible pip")
                } else {
                    print("Pip not possible")
                }
            } else {
